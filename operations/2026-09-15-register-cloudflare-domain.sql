...
do $pickpoint_cloudflare$
declare
  v_tenant_id constant uuid := '3a4bcfeb-e8a7-417a-8b0c-90c37c3a6175';
  v_hostname constant text := 'pickpoint-pickleclub.christianjelarjoyhisola.workers.dev';
  v_result jsonb;
...
begin
  ...
  v_result := public.provision_tenant_domain(
    'pickpoint-cloudflare-domain-20260915-a684e2d8818941a3a4e43165115fb516',
    v_tenant_id,
    v_hostname,
    false
  );
  ...
  if v_result ->> 'hostname' <> v_hostname
    ...
    raise exception 'PICKPOINT_CLOUDFLARE_DOMAIN_POSTCONDITION_FAILED';
  end if;
