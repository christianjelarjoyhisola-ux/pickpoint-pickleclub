...
function CompleteBookingSummary({ confirmation, bookingDate, defaultExpanded = false }: { confirmation: BookingConfirmation; bookingDate: string; defaultExpanded?: boolean }) {
  ...
  return <details className="pp-selection-review pp-complete-summary" open={expanded} onToggle={(event) => setExpanded(event.currentTarget.open)}>
    ...
    <dl className="pp-price-breakdown"><div><dt>Court time</dt><dd>{money(confirmation.totalAmount, confirmation.currency)}</dd></div><div><dt>Booking fee<small>Already included in the court price</small></dt><dd><span className="pp-free-fee">FREE</span></dd></div><div><dt>Total due</dt><dd>{money(confirmation.totalAmount, confirmation.currency)}</dd></div></dl>
  </details>;
}
