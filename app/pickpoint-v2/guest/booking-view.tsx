    ...
    <GuestShell current="book" checkout={step === "details" || step === "method" || step === "payment"}>
      ...
      <div id="booking-times" className={`pp-book-layout${holdIntro ? " is-hold-intro" : ""}`}>
        ...
        {step === "details" && selectedSlots.length > 0 && confirmation && (
          <form className="pp-book-card pp-details" onSubmit={saveDetails}>
            ...
            {(data?.capabilities?.weatherCreditsV1 || confirmation.weatherCreditAmount) && <details className="pp-weather-credit" open={confirmation.weatherCreditAmount ? true : undefined}>
              <summary>{confirmation.weatherCreditAmount ? "Booking credit applied" : "Have a booking credit?"}<span>No account needed</span></summary>
              ...
              <small>Booking credit from affected slots covers the booking total, including the booking fee. Older vouchers may cover court charges only.</small></div>
            </details>}
        ...
        {step === "done" && confirmation && (
          <section className="pp-book-card pp-confirmed is-submitted"><div className="pp-checkmark"><Check /></div><p className="pp-kicker">{confirmation.status === "confirmed" ? "You’re booked" : "Receipt submitted"}</p><h2>{confirmation.reference}</h2><p>{confirmation.status === "confirmed" ? "Your booking credit has covered this booking. Your court time is confirmed—see you on court." : "Your payment receipt has been submitted for review. Once approved, you’ll receive your booking confirmation. Keep this reference to check your status."}</p>{creditEmailPending && <p>Your booking is confirmed, but the email could not be sent yet. Keep this reference.</p>}<dl><div><dt>Court</dt><dd>{confirmation.courtName}</dd></div><div><dt>Total</dt><dd>{money(confirmation.totalAmount, confirmation.currency)}</dd></div><div><dt>Payment</dt><dd>{confirmation.status === "confirmed" ? "Paid with booking credit" : "Receipt submitted"}</dd></div><div><dt>Booking</dt><dd>{bookingStatusLabel(confirmation.status === "confirmed" ? "confirmed" : receiptOutcome?.booking.status)}</dd></div></dl>{Boolean(confirmation.weatherCreditAmount) && <p className="pp-credit-balance">{money(confirmation.weatherCreditAmount!, confirmation.currency)} credit used · {money(confirmation.weatherCreditBalance || 0, confirmation.currency)} left on your voucher.</p>}<div className="pp-actions"><button className="pp-button pp-button-outline" onClick={resetSelection}>Book another time</button><Link className="pp-button pp-button-blue" href="/book?mode=manage">View this booking</Link></div></section>
        )}
      </div>
    </GuestShell>
  );
