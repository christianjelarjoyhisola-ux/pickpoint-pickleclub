-- Extend PickPoint's tenant-locked public availability with a distinct
-- payment-review state. No booking details or other tenant rows are exposed.
    ...
    jsonb_build_object(
      ...
      'unavailable', coalesce((
        select jsonb_agg(jsonb_build_object(
          ...
          'state', case
            when occupancy.status = 'confirmed' then 'booked'
            when coalesce((
              select booking.payment_status
              ...
              where occupancy.source_kind = 'booking_slot'
                and slot.tenant_id = occupancy.tenant_id
                and slot.id = occupancy.source_id
              limit 1
